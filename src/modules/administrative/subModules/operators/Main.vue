  ...
  <div class="">
    ...
    <nav class="level">
      ...
      <div class="level-right">
        <p class="level-item" v-if="isNotEmpty(booleanColumns)"><a :class="getCSSState() + ' is-hidden-mobile'" @click="people_removeAllBooleanFilter([])"><strong>Todos</strong></a></p>
        <p class="level-item"
           v-for="(col, index) in booleanColumns">
          <a :class="getCSSState() + ' is-hidden-mobile'" @click="localAddBooleanFilter(index)" v-if="isBooleanApplied(index) === false">{{ col.label }}</a>
...
export default {
  ...
  methods: {
    ...
    ...mapActions([
      'operators_updateCurrentPag',
      'operators_updateTotalDocs',
      'operators_updateFiltersSearch',
      'operators_addBooleanFilter',
      'operators_removeBooleanFilter',
      'operators_removeAllBooleanFilter',
      'operators_addSortColumn'
    ]),
    newDocument () {
    ...
    localUpdateSearchFilters () {
      ...
      this.operators_updateFiltersSearch({ text: _search.text, fieldName: _search.fieldName, state: 'applied' })
    },
    localRemoveBooleanFilter (field) {
      this.operators_removeBooleanFilter(field)
    },
    localAddBooleanFilter (field) {
      this.operators_addBooleanFilter(field)
    },
    ...
    clearSearchFields () {
      ...
      this.operators_updateFiltersSearch(_obj) // eh uma mutations invocada por uma action no vuex
    },
    changePag (pag) {
      if (pag !== undefined) {
        this.operators_updateCurrentPag(pag)
      }
      this.getAll()
    },
    isSearchFilter (col) {
    ...
    localAddSortColumn (column) {
      if (this.isLoading() === false) {
        this.control.disableSortColumns = true
        this.operators_addSortColumn({ field: column, sort: this.getSortColumnState(column) })
        this.changePag(1)
      }
    },
    ...
    getAll () {
      ...
      this.$http.get(_uri).then((response) => {
        this.operators_updateTotalDocs(response.headers.get('X-Total-Count'))
  ...
  computed: {
    ...mapState({
      ...
      pagination: state => {
        const { pagination } = state.operators
        return pagination
      },
      filters: state => {
        const { filters } = state.operators
        return filters
      },
      sort: state => {
        let _arr = []
        const { sort } = state.operators
  ...
  watch: {
    'filters.search.state' (val, oldVal) {
      this.operators_updateCurrentPag(1)
      this.getAll()
    },

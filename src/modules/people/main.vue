      ...
      <div class="container">
        ...
        <nav class="level">
          ...
          <div class="level-right">
            <p class="level-item" v-if="!_.isEmpty(booleanColumns)"><a :class="getCSSState()" @click="people_removeAllBooleanFilter([])"><strong>Todos</strong></a></p>
            <p class="level-item"
...
export default {
  ...
  methods: {
    ...mapActions([
      'people_updateCurrentPag',
      'people_updateTotalDocs',
      'people_updateFiltersSearch',
      'people_addBooleanFilter',
      'people_removeBooleanFilter',
      'people_removeAllBooleanFilter',
      'people_addSortColumn'
    ]),
    newDocument () {
    ...
    localUpdateSearchFilters () {
      ...
      this.people_updateFiltersSearch({ text: _search.text, fieldName: _search.fieldName, state: 'applied' })
    },
    localRemoveBooleanFilter (field) {
      this.people_removeBooleanFilter(field)
    },
    localAddBooleanFilter (field) {
      this.people_addBooleanFilter(field)
    },
    ...
    clearSearchFields () {
      ...
      this.people_updateFiltersSearch(_obj) // eh uma mutations invocada por uma action no vuex
    },
    changePag (pag) {
      if (pag !== undefined) {
        this.people_updateCurrentPag(pag)
      }
      this.getAll()
    },
    isSearchFilter (col) {
    ...
    localAddSortColumn (column) {
      if (this.isLoading() === false) {
        this.control.disableSortColumns = true
        this.people_addSortColumn({ field: column, sort: this.getSortColumnState(column) })
        this.changePag(1)
      }
    },
    ...
    getAll () {
      ...
      this.$http.get(_uri).then((response) => {
        this.people_updateTotalDocs(response.headers.get('X-Total-Count'))
  ...
  computed: {
    ...mapState({
      ...
      pagination: state => {
        const { pagination } = state.people
        return pagination
      },
      filters: state => {
        const { filters } = state.people
        return filters
      },
      sort: state => {
        let _arr = []
        const { sort } = state.people
  ...
  watch: {
    'filters.search.state' (val, oldVal) {
      this.people_updateCurrentPag(1)
      this.getAll()
    },

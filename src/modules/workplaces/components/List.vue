  ...
  <div class="">
    <nav class="level">
      ...
      <div class="level-right">
        <p class="level-item" v-if="isNotEmpty(booleanColumns)"><a :class="getCSSState()" @click="workplaces_removeAllBooleanFilter([])"><strong>Todos</strong></a></p>
        <p class="level-item"
...
export default {
  ...
  methods: {
    ...
    ...mapActions([
      'workplaces_updateCurrentPag',
      'workplaces_updateTotalDocs',
      'workplaces_updateFiltersSearch',
      'workplaces_addBooleanFilter',
      'workplaces_removeBooleanFilter',
      'workplaces_removeAllBooleanFilter',
      'workplaces_addSortColumn'
    ]),
    newDocument () {
      this.$router.push({ name: 'workplaces.new', params: { state: 'new' } })
    },
    updateDocument (doc) {
      this.$router.push({ name: 'workplaces.update', params: { state: 'update', workplaceId: doc._id } })
    },
    ...
    localUpdateSearchFilters () {
      ...
      this.workplaces_updateFiltersSearch({ text: _search.text, fieldName: _search.fieldName, state: 'applied' })
    },
    localRemoveBooleanFilter (field) {
      this.workplaces_removeBooleanFilter(field)
    },
    localAddBooleanFilter (field) {
      this.workplaces_addBooleanFilter(field)
    },
    ...
    clearSearchFields () {
      ...
      this.workplaces_updateFiltersSearch(_obj) // eh uma mutations invocada por uma action no vuex
    },
    changePag (pag) {
      if (pag !== undefined) {
        this.workplaces_updateCurrentPag(pag)
      }
      this.getAll()
    },
    isSearchFilter (col) {
      ...
      switch (col.type) {
        ...
        case 'reference':
          if (doc[index] !== undefined) {
            if (doc[index].name.length > this.config.grid.textCropLength) {
              return doc[index].name.substring(0, this.config.grid.textCropLength - 3) + '...'
            }
          }
    ...
    localAddSortColumn (column) {
      if (this.isLoading() === false) {
        this.control.disableSortColumns = true
        this.workplaces_addSortColumn({ field: column, sort: this.getSortColumnState(column) })
        this.changePag(1)
      }
    },
  ...
  computed: {
    ...mapState({
      ...
      API: state => {
        const { API } = state.workplaces
        return API
      },
      collection: state => {
        const { collection } = state.workplaces
        return collection
      },
      ...
      pagination: state => {
        const { pagination } = state.workplaces
        return pagination
      },
      filters: state => {
        const { filters } = state.workplaces
        return filters
      },
      sort: state => {
        let _arr = []
        const { sort } = state.workplaces
  ...
  watch: {
    'filters.search.state' (val, oldVal) {
      this.workplaces_updateCurrentPag(1)
      this.getAll()
    },
